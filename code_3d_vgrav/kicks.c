...
#endif
                //double t_damp = 0.05 / (1. + pow(All.Time/10.,2));
                double t_damp = 0.25 / (1. + pow(All.Time/3.,2)) + 0.05 / (1. + pow(All.Time/10.,1)) + 0.01 / (1. + pow(All.Time/30.,1));
                P[i].Vel[0] *= exp(-t_damp); // coefficient is constant per-timestep: adjust to make as aggressive or weak as desired //
