...
int does_particle_need_to_be_split(int i)
{
    ...
    if(P[i].Type == 3)
    {
        ...
        {
            /* deterministic */
            if( (long)((double)All.Time/((double)All.Grain_Spawn_Time_PerParticle)) > (long)P[i].Number_Spawned)
            {
                P[i].Number_Spawned += 1;
                return 1;
            }
        } else {        
            /* stochastic */
            double p = dt / All.Grain_Spawn_Time_PerParticle;
            if(get_random_number(i+2343453*ThisTask) < p) return 1;
        }
        }
    }
#endif
...
#endif
#ifdef PEBBLE_ACCRETION_TESTPROBLEM
    if(P[i].ID==0 && P[i].Type!=0) 
    {
        // initialize grains with NSH drift solution // 
        // P[i].Vel[BOX_SHEARING_PHI_COORDINATE] -= All.Pressure_Gradient_Accel / (2. * BOX_SHEARING_OMEGA_BOX_CENTER); // this is the gas solution, for no back-reaction
        double v0 = -All.Pressure_Gradient_Accel / (2. * BOX_SHEARING_OMEGA_BOX_CENTER);
        double v_K = -(P[j].Pos[0]-boxHalf_X) * BOX_SHEARING_Q*BOX_SHEARING_OMEGA_BOX_CENTER;
        //double v_K = -(2./3.*boxHalf_X) * BOX_SHEARING_Q*BOX_SHEARING_OMEGA_BOX_CENTER; // use this radius because it is where the grains are allowed to 're-couple' to the flow
        double tau_s = (1./1.59577) * P[j].Grain_Size * BOX_SHEARING_OMEGA_BOX_CENTER;
        // tS = (sqrt(GAMMA)/1.59577) * (R_grain_code * rho_grain_code) / (rho_gas * cs); // working in units with rho=1, cs=1 where spawned, gamma=1, rho_grain_code=1 //
        v0 /= (1. + tau_s*tau_s); // appears in both terms here //
        P[j].Vel[0] = v0 * 2.*tau_s; // radial drift
        P[j].Vel[BOX_SHEARING_PHI_COORDINATE] = v_K + v0; // azimuthal drift relative to keplerian frame
    }
#endif
