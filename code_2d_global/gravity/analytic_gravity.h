...
void GravAccel_ShearingSheet()
{
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
         ...
         //if( (P[i].Type==0) || ((P[i].Pos[0]-boxHalf_X)/boxHalf_X<2./3.) )
#endif
...
#endif
#ifdef PEBBLE_ACCRETION_TESTPROBLEM
        if((P[i].Type==3) && (P[i].ID>0) && ((P[i].Pos[0]-boxHalf_X)/boxHalf_X>2./3.))
        {
            P[i].GravAccel[0]=P[i].GravAccel[1]=P[i].GravAccel[2]=0;
            ...
            double tau_s = (1./1.59577) * P[i].Grain_Size * BOX_SHEARING_OMEGA_BOX_CENTER;
        ...
        //printf("\n I MADE IT HERE! \n");
        //Temporarily commented out to check initial conditions (verify for myself) -- but now its back
