...
#ifndef PEBBLE_ACCRETION_TESTPROBLEM
        P[i].Number_Spawned = 0;
		{ 	
...
if(2==2)
{
P[i].Vel[BOX_SHEARING_PHI_COORDINATE] = -(P[i].Pos[0]-boxHalf_X) * BOX_SHEARING_Q*BOX_SHEARING_OMEGA_BOX_CENTER;
if(P[i].Type==0)
{
   P[i].Vel[BOX_SHEARING_PHI_COORDINATE] -= All.Pressure_Gradient_Accel / (2. * BOX_SHEARING_OMEGA_BOX_CENTER);
}
if(P[i].Type==3)
{
    P[i].ID=0; P[i].Vel[0]=0;
/*
  if(P[i].Pos[0]<0.66*All.BoxSize) {P[i].Mass=0;}
  //if(P[i].Pos[0]>0.95*All.BoxSize) {P[i].ID=0;}
  if(P[i].Pos[0]>0.90*All.BoxSize) {P[i].ID=0; P[i].Vel[0]=0;}
*/
}
} else {
if(P[i].Type==3 && P[i].ID>0)
{
//P[i].Vel[0]-=2.*All.Pressure_Gradient_Accel*P[i].Grain_Size/(1.+P[i].Grain_Size);
//EJL: typo in the original code
    P[i].Vel[0]-=2.*All.Pressure_Gradient_Accel*P[i].Grain_Size/(1.+P[i].Grain_Size * P[i].Grain_Size);
}
}
}
#endif
